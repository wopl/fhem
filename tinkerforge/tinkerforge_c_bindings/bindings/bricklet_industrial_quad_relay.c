...
void industrial_quad_relay_create(IndustrialQuadRelay *industrial_quad_relay, const char *uid, IPConnection *ipcon) {
	device_create(industrial_quad_relay, uid, ipcon, 2, 0, 0);

	industrial_quad_relay->response_expected[INDUSTRIAL_QUAD_RELAY_FUNCTION_SET_VALUE] = DEVICE_RESPONSE_EXPECTED_FALSE;
	industrial_quad_relay->response_expected[INDUSTRIAL_QUAD_RELAY_FUNCTION_GET_VALUE] = DEVICE_RESPONSE_EXPECTED_ALWAYS_TRUE;
	industrial_quad_relay->response_expected[INDUSTRIAL_QUAD_RELAY_FUNCTION_SET_MONOFLOP] = DEVICE_RESPONSE_EXPECTED_FALSE;
	industrial_quad_relay->response_expected[INDUSTRIAL_QUAD_RELAY_FUNCTION_GET_MONOFLOP] = DEVICE_RESPONSE_EXPECTED_ALWAYS_TRUE;
	industrial_quad_relay->response_expected[INDUSTRIAL_QUAD_RELAY_FUNCTION_SET_GROUP] = DEVICE_RESPONSE_EXPECTED_FALSE;
	industrial_quad_relay->response_expected[INDUSTRIAL_QUAD_RELAY_FUNCTION_GET_GROUP] = DEVICE_RESPONSE_EXPECTED_ALWAYS_TRUE;
	industrial_quad_relay->response_expected[INDUSTRIAL_QUAD_RELAY_FUNCTION_GET_AVAILABLE_FOR_GROUP] = DEVICE_RESPONSE_EXPECTED_ALWAYS_TRUE;
	industrial_quad_relay->response_expected[INDUSTRIAL_QUAD_RELAY_CALLBACK_MONOFLOP_DONE] = DEVICE_RESPONSE_EXPECTED_ALWAYS_FALSE;
	industrial_quad_relay->response_expected[INDUSTRIAL_QUAD_RELAY_FUNCTION_SET_SELECTED_VALUES] = DEVICE_RESPONSE_EXPECTED_FALSE;
	industrial_quad_relay->response_expected[INDUSTRIAL_QUAD_RELAY_FUNCTION_GET_IDENTITY] = DEVICE_RESPONSE_EXPECTED_ALWAYS_TRUE;

	industrial_quad_relay->callback_wrappers[INDUSTRIAL_QUAD_RELAY_CALLBACK_MONOFLOP_DONE] = (void *)industrial_quad_relay_callback_wrapper_monoflop_done;
}
...
int industrial_quad_relay_set_response_expected(IndustrialQuadRelay *industrial_quad_relay, uint8_t function_id, bool response_expected) {
	return device_set_response_expected(industrial_quad_relay, function_id, response_expected);
}

int industrial_quad_relay_set_response_expected_all(IndustrialQuadRelay *industrial_quad_relay, bool response_expected) {
	return device_set_response_expected_all(industrial_quad_relay, response_expected);
}
...
void industrial_quad_relay_register_callback(IndustrialQuadRelay *industrial_quad_relay, uint8_t id, void *callback, void *user_data) {
	device_register_callback(industrial_quad_relay, id, callback, user_data);
}

int industrial_quad_relay_get_api_version(IndustrialQuadRelay *industrial_quad_relay, uint8_t ret_api_version[3]) {
	return device_get_api_version(industrial_quad_relay, ret_api_version);
}
...
int industrial_quad_relay_set_value(IndustrialQuadRelay *industrial_quad_relay, uint16_t value_mask) {
	...
	ret = packet_header_create(&request.header, sizeof(request), INDUSTRIAL_QUAD_RELAY_FUNCTION_SET_VALUE, industrial_quad_relay->ipcon, industrial_quad_relay);
	...
	ret = device_send_request(industrial_quad_relay, (Packet *)&request, NULL);
...
int industrial_quad_relay_get_value(IndustrialQuadRelay *industrial_quad_relay, uint16_t *ret_value_mask) {
	...
	ret = packet_header_create(&request.header, sizeof(request), INDUSTRIAL_QUAD_RELAY_FUNCTION_GET_VALUE, industrial_quad_relay->ipcon, industrial_quad_relay);
	...
	ret = device_send_request(industrial_quad_relay, (Packet *)&request, (Packet *)&response);
...
int industrial_quad_relay_set_monoflop(IndustrialQuadRelay *industrial_quad_relay, uint16_t selection_mask, uint16_t value_mask, uint32_t time) {
	...
	ret = packet_header_create(&request.header, sizeof(request), INDUSTRIAL_QUAD_RELAY_FUNCTION_SET_MONOFLOP, industrial_quad_relay->ipcon, industrial_quad_relay);
...
int industrial_quad_relay_get_monoflop(IndustrialQuadRelay *industrial_quad_relay, uint8_t pin, uint16_t *ret_value, uint32_t *ret_time, uint32_t *ret_time_remaining) {
	...
	ret = packet_header_create(&request.header, sizeof(request), INDUSTRIAL_QUAD_RELAY_FUNCTION_GET_MONOFLOP, industrial_quad_relay->ipcon, industrial_quad_relay);
...
int industrial_quad_relay_set_group(IndustrialQuadRelay *industrial_quad_relay, char group[4]) {
	...
	ret = packet_header_create(&request.header, sizeof(request), INDUSTRIAL_QUAD_RELAY_FUNCTION_SET_GROUP, industrial_quad_relay->ipcon, industrial_quad_relay);
...
int industrial_quad_relay_get_group(IndustrialQuadRelay *industrial_quad_relay, char ret_group[4]) {
	...
	ret = packet_header_create(&request.header, sizeof(request), INDUSTRIAL_QUAD_RELAY_FUNCTION_GET_GROUP, industrial_quad_relay->ipcon, industrial_quad_relay);
...
int industrial_quad_relay_get_available_for_group(IndustrialQuadRelay *industrial_quad_relay, uint8_t *ret_available) {
	...
	ret = packet_header_create(&request.header, sizeof(request), INDUSTRIAL_QUAD_RELAY_FUNCTION_GET_AVAILABLE_FOR_GROUP, industrial_quad_relay->ipcon, industrial_quad_relay);
...
int industrial_quad_relay_set_selected_values(IndustrialQuadRelay *industrial_quad_relay, uint16_t selection_mask, uint16_t value_mask) {
	...
	ret = packet_header_create(&request.header, sizeof(request), INDUSTRIAL_QUAD_RELAY_FUNCTION_SET_SELECTED_VALUES, industrial_quad_relay->ipcon, industrial_quad_relay);
...
int industrial_quad_relay_get_identity(IndustrialQuadRelay *industrial_quad_relay, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
	...
	ret = packet_header_create(&request.header, sizeof(request), INDUSTRIAL_QUAD_RELAY_FUNCTION_GET_IDENTITY, industrial_quad_relay->ipcon, industrial_quad_relay);
