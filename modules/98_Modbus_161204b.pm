...
#
#   ToDo / Ideas : 
#                   special handling for broken modbus fc3 replies -> attr
#                   scanner für ids
...
my $Modbus_Version = '3.5.4 - 4.12.2016';
    ...
    if ($fCode >= 128) {                                    # error
        ...
        if ($headerLen > $actualLen) {
            if ($eCRC != $CRC) {
                ...
                Log3 $name, 5, "$name: ParseFrames: frame seems incomplete ($actualLen / $headerLen) but checksum is fine ...";
            }
        }
...
sub Modbus_TimeoutSend($)
{
    ...
    Log3 $name, 3, "$name: timeout waiting for fc $ioHash->{REQUEST}{FCODE} " .
...
sub Modbus_HandleSendQueue($;$)
{
    ...
    Log3 $name, 4, "$name: HandleSendQueue sends fc $fCode to id $reqId, tid $tid for $reading ($type$adr), len $len" .
                    ", device $logHash->{NAME} ($proto), pdu " . unpack ('H*', $pdu);
...
sub ModbusLD_Send($$$;$$$){
    ...
    my $fCode = ModbusLD_DevInfo($hash, $type, $fcKey, $defaultFCode{$type}{$fcKey}); 
    if (!$fCode) {
